...
// If compiled for Unicode
		...
		else
		{
			...
			if (bReadData)
			{
				...
				nChars = GetUnicodeStringFromMultiByteString(pszMultiByteString, pszUnicodeString, nMAX_LINE_CHARS, m_is_utf8_Text?CP_UTF8:m_nFileCodePage);
...
/*static*/ bool CStdioFileEx::IsFileUnicode(const CString& sFilePath)
{
	...
	m_is_utf8_Text = false;
	//wchar_t			cFirstChar;
	//CFileException	exFile;
	
	DDString::CZStringA str;
	str.SetData(sFilePath,wcslen(sFilePath));
	FILE *fp;

	char buf[3];
	memset(buf,0,sizeof(buf));
	fp = fopen(str.GetData(),"r");
	if(fp)
	{
		fread(buf,3,1,fp) ;
		if((0xFF == (int)(unsigned char)buf[0] && 0xFE ==(int)(unsigned char) buf[1]))			
			bIsUnicode = true;
		else if((0xEF == (int)(unsigned char)buf[0] && 0xBB == (int)(unsigned char)buf[1] && 0xBF == (int)(unsigned char)buf[2]))
			m_is_utf8_Text = true;
		fclose(fp);
	}
